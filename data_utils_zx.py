...
def get_images(root):
    ...
    for i in range(len(files)):
        ...
        name.append(os.path.basename(files[i]))
...
def load_annoataion_txt(p):
    ...
    with open(p, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in line]
            # print(line)
            x1, y1, x2, y2, x3, y3, x4, y4 = list(map(int, [float(x) for x in line[:8]]))
            text_polys.append([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
            if label == '*' or label == '###':
                text_tags.append(True)
            else:
...
def image_label(txt_root, 
    ...
    text_polys, text_tags = load_annoataion_txt(txt_fn)
...
class custom_dset(data.Dataset):
    def __init__(self, img_root, txt_root, vis = False):
        ...
        self._img_name_list = [img_name for img_name in os.listdir(img_root)]
        self._txt_path_list = [os.path.join(txt_root, txt_name) for txt_name in os.listdir(txt_root)]
        self._img_path_list = [os.path.join(img_root, img_name) for img_name in os.listdir(img_root)]
        self.txt_name_list = sorted(self._txt_name_list)
        self.img_name_list = sorted(self._img_name_list)
        self.txt_path_list = sorted(self._txt_path_list)
        self.img_path_list = sorted(self._img_path_list)
        for i in range(len(self.img_path_list)):
            print(i, self.img_path_list[i], self.img_name_list[i])
            print(i, self.txt_path_list[i], self.txt_name_list[i])
